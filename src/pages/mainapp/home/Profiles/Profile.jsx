...
function Profile(props) {
    ...
    const followaccount = async () => {
        ...
        if (data.flag === true) {
            toast("Followed")
            setfollowinglist(followinglist.push(profileid))
            console.log(followinglist)
    ...
    const unfollowaccount = async () => {
        ...
        let data = await fetch(`${process.env.REACT_APP_API_KEY}auth/unfollow/${profileid}`, {
            method: "PUT",
            headers: {
                'Content-Type': 'application/json',
                'authtoken': localStorage.getItem('sclmdia_73sub67_token')
        ...
        if (data.flag === true) {
            toast("UnFollowed")
            setfollowinglist(followinglist.pull(profileid))
            console.log(followinglist)
            ...
            <div className="mainbox">
                <div className="innermainbox">
                    <div className="name-dp-id">
                        <img className='dp987' src={userinfo?.profilepic !== null ? userinfo?.profilepic : "https://res.cloudinary.com/dbnqqpobe/image/upload/v1683888315/removal.ai__5c5595dd-bcec-47ef-9403-c6e5317ac359_uiregk.png"} alt="image" />
                        ...
                        <div className='follow-editprofile'>
                            {
                                ...
                                mydetails?._id === userinfo?._id ?
                                    <button data-bs-toggle="modal" data-bs-target="#editprofilemodal" onClick={() => { changepage(5) }}>Edit Profile</button>
                                    : ((userinfo?.following.includes(mydetails?._id) && !myinfo?.following.includes(userinfo._id)) ?
